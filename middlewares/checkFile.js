...
export const checkFile = (req, res, next) => {
  ...
  if (!file) {
    return res.status(400).json({ status: "error", data: "Please provide image file" });
  }

  const fileExt = path.extname(file.name).toLowerCase();
  if (!supportedExts.includes(fileExt)) {
    return res.status(400).json({ status: "error", data: "Please provide valid image file" });
  }
  ...
  file.mv(`./uploads/${imagePath}`, (err) => {
    if (err) {
      return res.status(500).json({ status: "error", data: "Image upload failed", message: err.message });
    }
...
export const updateCheckFile = (req, res, next) => {
  ...
  file.mv(`./uploads/${imagePath}`, (err) => {
    if (err) return res.status(500).json({ status: "error", data: "Image upload failed", message: err.message });
